...
static void lprf_sync_read_reg_debugging(struct lprf_local *lp, const u8 reg, struct lprf_state_change *ctx,  const bool output_disable)
{
	...
	if(!output_disable){	
		printk(KERN_DEBUG "lprf: lprf_async_read_reg_debugging - start: reg=%u  \n", reg);	
	}
	...
	if(!output_disable){	 
		printk(KERN_DEBUG "lprf: lprf_async_read_reg_debugging - end.\n");
	}	
}
...
lprf_async_state_assert(void *context)
{
	...
	printk(KERN_DEBUG "lprf: lprf_async_state_assert - start: *(ctx->buf + 0)=%u *(ctx->buf + 1)=%u *(ctx->buf + 2)=%u *(ctx->buf + 3)=%u ctx->from_state=%u\n", *(ctx->buf +0), *(ctx->buf+ 1), *(ctx->buf +2) , *(ctx->buf + 3), ctx->from_state);
...
static void lprf_async_state_change_complete(void *context){
	...
	if(lp->is_tx){
		...
		if(buffer & STATE_SENDING){
			...
			printk(KERN_DEBUG "lprf: lprf_async_state_change_complete - reset SM_MAIN. %s:%i\n",  __FILE__, __LINE__);	
...
{
	printk(KERN_DEBUG "lprf: lprf_start - initialise state change. %s:%i\n", __FILE__, __LINE__);
...
static int lprf_hw_init(struct lprf_local *lp)
{
	...
	rc = lprf_write_subreg(lp, SR_LDO_VCO_VOUT, 28);
	if (rc){
		return rc;
	}
	...
	rc = lprf_write_subreg(lp, SR_CTRL_CLK_ADC, 1);
	if (rc){
		return rc;
	}
	...
	rc = lprf_write_subreg(lp, SR_PLL_MOD_EN, 1);		//enable on-chip modulation
	if (rc){
		return rc;
	}
	...
	rc = lprf_write_subreg(lp, SR_TX_ON_CHIP_MOD_SP, 8);    //set TX_ON_CHIP_MOD_SP to ... (3: 250kHz; 8: 20kHz; 9: 10kHz)
	if (rc){
		return rc;
	}
...
//	printk(KERN_DEBUG "lprf: scull_write: abcdefghijklmnopqrstuvwxyz0123456789abcdefghijklmnopqrstuvwxyz0123456789\n");
//	printk(KERN_DEBUG "lprf: scull_write: abcdefghijklmnopqrstuvwxyz0123456789abcdefghijklmnopqrstuvwxyz0123456789\n");	
